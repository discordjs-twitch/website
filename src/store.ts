	...
	actions: {
		fetchStats: async ({ commit }) => {
			...
			const [fetchedDownloads, fetchedStars, fetchedContributors] = await Promise.all([
				fetch('https://api.npmjs.org/downloads/range/2022-03-10:2100-08-21/discordjs-twitch').then(
					toJSON,
					noop,
				),
				fetch('https://api.github.com/repos/discordjs-twitch/discordjs-twitch').then(toJSON, noop),
				fetch('https://api.github.com/repos/discordjs-twitch/discordjs-twitch/stats/contributors').then(toJSON, noop),
			]);
